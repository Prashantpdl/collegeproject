...
exports.verifyuser = async (req, res, next) => {
  ...
  if (userotp.otp == otp) {
    ...
    return res.status(200).send({ message: "User verified , Please login ",status:200 });
  } else {
    return res.status(404).send({ message: "Wrong OTP,Please try again ", status:404 });
  }
};
exports.checkOtp = async (req, res, next) => {
  ...
  try {
    ...
    if (!userToken) {
      return res.send({
        message: "Token not found, Please enter correct one",
        status: 404,
      });
    }
...
exports.deleteuser = async (req, res, next) => {
  try {
    ...
    const otp = await Otp.findOneAndDelete({email:user.email})
    res.status(204).json({
      status: "success",
      data: null,
    });
